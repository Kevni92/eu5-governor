...
# -----------------------------------------------------------------------------
# 1) Add component script values so the tooltip can explain the live monthly rate.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 2) Native confirmation for replace, all role changes and dismissal.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 3) Fix occupied portrait right click and split Entrenchment tooltip into sections.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 4) Localization with nested vanilla Game Concepts and readable sections.
# -----------------------------------------------------------------------------
loc_specs = {
...
for path, spec in loc_specs.items():
    loc = read(path)
    # Keep the old single dynamic key as a compact fallback, but make it structured too.
    if 'EU5GOV_ENTRENCHMENT_TT_GROWTH:' not in loc:
        anchor = ' EU5GOV_ENTRENCHMENT_TT_DYNAMIC:'
        lines = loc.splitlines()
        idx = next(i for i, line in enumerate(lines) if line.startswith(anchor))
        lines[idx+1:idx+1] = [' ' + spec['growth'], ' ' + spec['effects'], ' ' + spec['dismissal']]
        loc = '\n'.join(lines) + '\n'
    else:
...
# -----------------------------------------------------------------------------
# 5) README corrections relevant to current mechanics.
# -----------------------------------------------------------------------------
